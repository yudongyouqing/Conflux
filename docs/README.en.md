...
Codex CLI does not read the Claude Code `.mcp.json`. Its project-scope mount lives in the repo-root `.codex/config.toml` (honored for trusted projects only; this repository ships one):
...
On non-Windows platforms use `command = "npx"` with `args = ["tsx", "apps/server/src/index.ts", "mcp"]`. Approve project trust on the first Codex launch in a new directory, and fully restart the Codex host after changing the file.
