...
Hooks associate lifecycle events with session identity and keep liveness current, supporting `SessionStart`, `UserPromptSubmit`, and `Stop`; they also maintain custom titles, `/resume` lineage, and undelivered messages; nodes are named after the project directory (matching what the terminal tab shows, with a numeric suffix on collisions), and `/rename` overrides. Set `CLAUDE_CONFIG_DIR` when Claude Code uses a custom configuration directory. Installing also backfills Claude Code sessions that were already running at that moment (or run `hooks backfill` standalone); Codex backfill is not supported yet.
