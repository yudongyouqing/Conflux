...
- 后端继续监听 `127.0.0.1:9527`，Vite 代理目标固定使用 IPv4 地址，避免 Windows 上 `localhost` 的 IPv6 解析问题；Vite 使用 `--strictPort`，端口冲突时立即失败。
...
- 开发态 Electron 用户数据写入仓库内的 `.electron-dev`，并禁用硬件加速，适配无法写入默认 Electron 缓存目录或 GPU 不可用的开发环境；打包态保留系统默认路径和硬件加速设置。
...
apps/desktop/src/main.cjs
  ...
  ├─ spawn: npm run dev -w apps/web -- --host 127.0.0.1 --strictPort
...
`apps/desktop/src/dev-services.cjs` 只负责开发服务定义、HTTP 就绪检测、子进程退出判定和停止逻辑，保持 Electron 生命周期代码可测试。`runtime-config.cjs` 负责开发态 Electron 用户数据和图形运行时设置。`main.cjs` 负责 Electron 窗口、启动顺序、错误展示和退出清理。`preload.cjs` 不承载业务数据访问，避免把 Node 能力直接暴露给 React 页面。
...
4. 主进程轮询后端健康检查和 Vite 根页面；任一服务在超时前没有响应，或子进程提前退出且地址不可访问时，显示明确错误并退出。
...
- Vite/Fastify 子进程提前退出时立即结束等待；如果对应地址仍可访问，则复用已存在的服务。
...
- 单元测试验证子进程提前退出时快速失败，以及开发态用户数据目录和硬件加速配置。
