...
修改配置后必须完全重启 MCP 宿主（例如 Claude Code 或其他客户端），仅刷新网页不会重新建立 stdio 连接。确认使用的是当前仓库路径，并检查宿主的 stderr 日志；MCP 的 stdout 只应包含 JSON-RPC 数据。
...
Keep exactly one Conflux server entry in the project `.mcp.json`. New projects should use the `conflux` key; older projects may keep the single `muiltchat` key. Restart the MCP host completely after changing the file. Reloading the web page does not recreate a stdio connection.
