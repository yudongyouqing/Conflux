...
Hooks 把会话生命周期事件与身份关联起来并持续更新在线状态，支持 `SessionStart`、`UserPromptSubmit`、`Stop` 三类事件；同时维护自定义标题、`/resume` 会话继承和未投递消息；节点名跟随 Claude Code 自己的会话名（终端标签显示的那个，`/rename` 后自动同步）。Claude Code 使用自定义配置目录时，通过 `CLAUDE_CONFIG_DIR` 指定。安装时会自动补录当时已在运行的 Claude Code 会话（也可单独执行 `hooks backfill`）；Codex 会话暂不支持补录。
